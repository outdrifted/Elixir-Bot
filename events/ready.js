...
const steam_apikey = process.env.STEAM_API_KEY;
	...
	async execute(client) {
		...
		(function VCChecker() {
			setTimeout(async function () {
				//var r = await db.get('vcs.243436321018871810');
				//console.log(r);
				Object.keys(guilds).forEach(async (guildID) => {
					...
					for (const [key, value] of channels.entries()) {
						...
						if (key != guilds[guildID].vc.channel) {
							...
							// check if user has been in 'create vc' for at least 5 seconds
